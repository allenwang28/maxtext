...
class MaxTextTrainer:
    ...
    def process_config(
        ...
        # an argv route
        argv = ["MaxText.py", base_yaml_fpath, f"run_name={run_name}",]
        ...
        self.diagnostic_config = diagnostic_configuration.DiagnosticConfig(
            debug_config)
        self.config = config
    ...
    def train(self, num_steps: Optional[int] = None) -> int:
        ...
        with diagnostic.diagnose(self.diagnostic_config):
            ...
            last_profiling_step = np.clip(first_profiling_step + self.config.profiler_steps - 1, first_profiling_step, self.config.steps - 1)
            ...
            for step in np.arange(start_step, stop_step):
                ...
                record_scalar_metrics(metrics, new_time - last_step_completion, per_device_tflops, self.learning_rate_schedule(step), per_device_tokens)
